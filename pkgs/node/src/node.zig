            ...
            const missing_roots = self.chain.onBlock(signed_block.*, .{}) catch |err| {
                ...
                if (err == chainFactory.BlockProcessingError.MissingPreState) {
                    ...
                    if (current_depth >= constants.MAX_BLOCK_FETCH_DEPTH) {
                        self.logger.err(
                            "FORK DIVERGENCE LIKELY: Reached max block fetch depth ({d}) for block 0x{s} at slot {d} without finding common ancestor. " ++
                    ...
                    if (self.cacheBlockAndFetchParent(block_root, signed_block.*, current_depth + 1)) |parent_root| {
                        ...
                        if (cache_err == CacheBlockError.PreFinalized) {
                            ...
                            if (!have_parent) {
                                self.logger.err(
                                    "FORK DIVERGENCE DETECTED: block 0x{s} at slot {d} is pre-finalized but parent 0x{s} not in our chain. " ++
                        ...
                        .status => |*status_ctx| {
                            ...
                            switch (sync_status) {
                                ...
                                .fork_diverged => |diverge_info| {
                                    self.logger.err("FORK DIVERGENCE DETECTED: our finalized=0x{s} at slot {d}, peer finalized=0x{s} at slot {d}. Checkpoint sync required to recover.", .{
                                        std.fmt.fmtSliceHexLower(&diverge_info.our_finalized_root),
                                        diverge_info.our_finalized_slot,
                                        std.fmt.fmtSliceHexLower(&diverge_info.peer_finalized_root),
